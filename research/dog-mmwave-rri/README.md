...
**実装・実験は別リポジトリ [`dog-radar-vitals`](../../../dog-radar-vitals/) で行う（2026-07-22 分離）。**
このディレクトリは文献調査・テーマ設計を担い、モデルの試行錯誤・学習コード・実行結果は
`dog-radar-vitals` の責務とする。データセット本体（Ahmed et al. 2024 シナリオ1）も
`dog-radar-vitals/data/raw/` へ移した。
...
**UWB（Ahmed et al. 2024）は取得済み（2026-07-22）で、[`dog-radar-vitals/data/raw/`](../../../dog-radar-vitals/data/raw/) に配置している。**
ミリ波（MMECG）と CW（Schellenberger）は未取得である。
...
UWBデータセット（Ahmed et al. 2024 シナリオ1）を取得し、実装リポジトリ
[`dog-radar-vitals`](../../../dog-radar-vitals/) で素朴なTransformerによる
心拍数・呼吸数予測のベースラインを実装済み（2026-07-22）。実験の進捗は
そちらの [`EXPERIMENTS.md`](../../../dog-radar-vitals/EXPERIMENTS.md) を参照。
