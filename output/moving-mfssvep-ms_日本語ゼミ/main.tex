...
\parallelitem{We aimed to develop a moving multifocal steady-state visual evoked potential (mfSSVEP) stimulus to simultaneously assess afferent and efferent dysfunction in MS for application on a mobile platform.}{そこで本論文では，MS患者における求心性および遠心性の機能をモバイル環境で同時に評価することを目的とした．この評価を実現するために，我々は，多焦点定常状態視覚誘発電位（multifocal steady-state visual evoked potential; mfSSVEP）を誘発する移動視覚刺激を開発した．}

\parallelitem{The brain-computer interface (BCI) platform consists of a head-mounted virtual reality headset with electroencephalogram (EEG) and electrooculogram (EOG) sensors.}{この移動刺激を呈示する頭部装着型のバーチャルリアリティヘッドセットと，脳波（Electroencephalogram; EEG）センサおよび眼電図（Electrooculogram; EOG）センサを統合して，ブレイン・コンピュータ・インタフェース（Brain-Computer Interface; BCI）プラットフォームを構成した．}

\parallelitem{To evaluate the platform, we recruited consecutive patients who met the 2017 MS McDonald diagnostic criteria and healthy controls for a pilot cross-sectional study.}{このプラットフォームを評価するため，パイロット横断研究を実施した．この研究では，カリフォルニア大学サンディエゴ校のMSセンターにおいて，一定期間内に2017年版マクドナルド診断基準を満たしたMS患者を，選別せずに全員対象とした．比較のための健常対照者も対象とした．}
...
\parallelitem{The moving stimulus showed a reliable capability to assess both afferent and efferent visual functions simultaneously.}{この移動刺激を呈示することで，求心性と遠心性の視覚機能を，同時にかつ高い信頼性で評価できることを示した．}
